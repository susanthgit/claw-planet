...
1. **DM policy = pairing** — unknown senders never reach the model in the first place ([§6.1 check 2](/security/self-hosting-checklist/))
2. **Sandbox non-main** — if the message does reach the agent, the `read` tool runs inside Docker with no access to host secrets ([§6.1 check 4](/security/self-hosting-checklist/))
...
This is the [§4.1 Slack Connector](/plugins/) field note's headline risk: "default scopes grant more workspace access than most agents actually need."
...
- [§6.1 Self-hosting checklist](/security/self-hosting-checklist/) — the 12 checks
- [§6.4 What NOT to build as an agent](/security/what-not-to-build/) — patterns that should be skipped entirely
- [§6.2 Plugin trust signals](/security/plugin-trust-signals/) — ten signals applied to skills/MCPs
- [§1.4 Honest drawbacks](/overview/honest-drawbacks/) — the architectural drawbacks behind these patterns
