...
- **Cold-start latency** in OpenClaw is real (see [§1.4 drawback #2](/overview/honest-drawbacks/)). A response in 5 seconds for an agent with significant workspace context is normal — not great for live chat.
...
- **Adversarial input becomes destructive.** [§6.3 pattern #1 prompt injection](/security/practical-patterns/) is mostly recoverable from when the agent has read-only scope. With write-to-production scope, "tell the agent to push a commit" becomes attack vector.
...
- [§5 Use cases](/use-cases/) — agentic patterns that DO work
- [§6.1 Self-hosting checklist](/security/self-hosting-checklist/) — the operational posture
- [§7 Compare](/compare/) — when OpenClaw's agent shape fits vs other shapes
- [§1.4 Honest drawbacks](/overview/honest-drawbacks/) — the architectural drawbacks behind several of these
