...
| `~/.claude/settings.json` | Personal config — default model, thinking budget, MCP server allowlist |
| `~/.claude/credentials.json` | Stored login token (subscription path) — never commit this anywhere |
| `~/.claude/sessions/` | Session history (transcripts of past conversations) |
| `<project>/CLAUDE.md` | Project-scoped instructions — Claude reads this on every session in this project |
| `<project>/.claude/` | Project-scoped slash commands + subagent definitions + MCP pins |

The first two are private. Don't share `credentials.json`. Do version-control `CLAUDE.md` and `.claude/` — they're meant to be team-shared (without secrets).
