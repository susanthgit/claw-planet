...
- Azure Container Apps ([§2.5](/openclaw/setup/azure/))
...
- You're optimising for cold-start speed (containerised processes carry some startup overhead vs native on the same host — we don't have a measured number for OpenClaw specifically).
...
- [§2.4 Linux server](/openclaw/setup/linux-server/) — the most common Docker host shape
- [§2.5 Azure](/openclaw/setup/azure/) — Container Apps shape (Docker-flavoured but managed)
- [§2.6 Raspberry Pi](/openclaw/setup/raspberry-pi/) — Pi can run Docker too if you prefer it over native install
- [§6.1 Self-hosting checklist](/openclaw/security/self-hosting-checklist/) — extra steps for Dockerised deploys
