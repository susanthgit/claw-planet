...
You want a $130 always-on assistant that lives on your desk. You don't need voice (Pi has no native voice node — see [§1.4 drawback #6](/overview/honest-drawbacks/)). You're either already on Pi or want the cheapest credible "I have an agent" path.
...
A Pi running an OpenClaw Gateway as a `systemd` user service, restarting on boot, on your home network with a static IP or hostname. Connected to whichever channels you use (Telegram is the easiest first; Slack works fine; iMessage requires a Mac in the chain — see [§3.2 Channels](/connections/channels/)).
...
The onboarder will create the workspace, prompt for channel pairing, and install a `systemd` user service. Same flow as [§2.4 Linux server step 3](/setup/linux-server/).
...
If the agent responds, the runtime is wired. Now pair the channels you use ([§3.2 Channels](/connections/channels/)).
...
- **Run the agent on a local 3B model** for one week, then swap back to Claude. Notice the quality difference. (See [§5 Use cases — Pi-hosted always-on home agent](/use-cases/) for a real recipe.)
...
- [§5.4 Pi-hosted always-on home agent](/use-cases/) (planned) — the full recipe
- [§6.1 Self-hosting checklist](/security/self-hosting-checklist/) — even at home, defaults matter
- [§3.2 Channels](/connections/channels/) — pair the messaging surfaces you use
- [§2.4 Linux server](/setup/linux-server/) — same shape for non-Pi Linux boxes
