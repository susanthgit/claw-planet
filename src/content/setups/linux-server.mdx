...
- A NAS running native Linux (vs. NAS-via-Docker which is [§2.7 Docker](/openclaw/setup/docker/))

> **What this isn't:** a turn-key managed deploy. You'll be the sysadmin. If you want managed-feeling, [§2.5 Azure](/openclaw/setup/azure/) Container Apps is the closer shape.
...
Should respond with the time and a sentence acknowledging the hostname. If yes, your runtime is wired correctly. Now pair the channels you actually use ([§3.2 Channels](/openclaw/connections/channels/)).
...
| Azure B2s (4GB) | $0 | ~$30/month → see [§2.5 Azure](/openclaw/setup/azure/) |
...
- **Run the agent with a different SOUL.md per project.** Use multi-agent routing — different Slack workspaces hit different agents with different personalities. See [§1.2 Concepts → multi-agent](/openclaw/overview/concepts/#multi-agent-routing).
...
- [§2.7 Docker](/openclaw/setup/docker/) — containerised version of this same shape
- [§2.5 Azure](/openclaw/setup/azure/) — managed-ier shapes if you want to skip the sysadmin
- [§3.2 Channels](/openclaw/connections/channels/) — pair the messaging surfaces you actually use
