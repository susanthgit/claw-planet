...
> **Microsoft disclosure.** Sush works at Microsoft. Microsoft makes Azure. We cover Azure as one of several reasonable cloud paths because it's a common ask, not because of who pays the bills. If your team's already on Azure with reserved-instance pricing, this page applies. If not, [§2.4 Linux server](/setup/linux-server/) on Hetzner / DigitalOcean / a Pi is usually cheaper for personal use.
...
| **Azure VM (B-series)** | A full Linux box you SSH into | Same as [§2.4 Linux server](/setup/linux-server/) but on Azure infrastructure |

> If you want neither — i.e., you're learning OpenClaw on the side and don't want to think about Azure at all — start on a laptop ([§2.3](/setup/laptop/)) or a Pi ([§2.6](/setup/raspberry-pi/)) and come back to Azure when you have a real reason.
...
Pick this if you want the [§2.4 Linux server](/setup/linux-server/) shape but on Azure infrastructure. Use cases: existing Azure presence, Entra ID-managed identity for the model provider, regional compliance.
...
It's literally [§2.4 Linux server](/setup/linux-server/) once the VM exists. Specifically:
...
- **Wire OpenClaw alerts to Microsoft Teams.** Pair Teams as a channel, set up a cron job that summarises Application Insights traces every morning, posts to a Teams channel. ([§3.2 Channels → Microsoft Teams](/connections/channels/)).
...
- **You actively don't want a Microsoft-tied tech stack.** Fair. Use [§2.4 Linux server](/setup/linux-server/).

## What to read next

- [§2.4 Linux server](/setup/linux-server/) — the underlying shape if you pick the VM path
- [§2.7 Docker](/setup/docker/) — image patterns that apply to Container Apps
- [§6.1 Self-hosting checklist](/security/self-hosting-checklist/) — production-shape posture
