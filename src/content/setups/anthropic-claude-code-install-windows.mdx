...
|---|---|---|
| Binary | `%LOCALAPPDATA%\Anthropic\ClaudeCode\claude.exe` (winget) | `~/.local/bin/claude` |
| Personal config | `%USERPROFILE%\.claude\settings.json` | `~/.claude/settings.json` |
| Credentials | `%USERPROFILE%\.claude\credentials.json` | `~/.claude/credentials.json` |
| Project config | `<project>\CLAUDE.md` + `<project>\.claude\` | `<project>/CLAUDE.md` + `<project>/.claude/` |
