...
| **Azure VM (B-series small)** | 2GB+ (B2s = 4GB) | Workable for text channels; no local model | None | ~$40–$100/month | Production-style deployment. See [§2.5 Azure](/openclaw/setup/azure/). |
| **Azure Container Apps** | configurable | Workable for stateless restarts; persistence needs care | None | scales with use | Serverless-ish path. See [§2.5 Azure](/openclaw/setup/azure/). |
...
| **NAS (Synology / TrueNAS / Unraid)** | depends on model | Possible via Docker — see [§2.7 Docker](/openclaw/setup/docker/) | None | already have it | Quiet 24/7 home server you already pay for in electricity. |

## What "fit" actually means

The Gateway runtime is Node 24 (or 22.14+). It needs:

- **~200–500MB of RAM at idle** (estimated, one Gateway, one agent — not measured by us yet)
...
> **The table below is illustrative — sourced inferences, not measurements.** Local-model RAM/disk numbers come from the model providers' published specs (Ollama / Hugging Face). The Gateway-side figures are educated guesses pending a real measurement pass; treat them as ranges, not guarantees.

| Workload | Extra RAM needed | Extra disk needed |
|---|---|---|
| Just the Gateway, text channels, API-based models | Negligible | Negligible |
| Adding 5+ MCP servers (filesystem, GitHub, browser) | +100–500MB (estimated) | Negligible |
| Long sessions accumulating JSONL transcripts | Negligible | ~10MB / month / active session (estimated) |
| Local model via Ollama (e.g. Llama 3.1 8B) | +6GB at minimum (model spec) | +5–10GB per model (model spec) |
| Local model larger (70B+) | +50GB+ and you need a decent GPU (model spec) | +40GB+ per model (model spec) |
| Voice features (TTS, ASR) | +500MB–1GB (estimated) | +1GB (estimated) |
...
2. **Do you want voice?** (Mac/iOS/Android only — see [§1.4 drawback #6](/openclaw/overview/honest-drawbacks/).)
3. **Do you want a local model?** (Then RAM and possibly GPU dominate; otherwise hosted API is fine.)
4. **What's the threat surface?** (Always-on internet-facing changes the security calculus — see [§6.1 Self-hosting checklist](/openclaw/security/self-hosting-checklist/).)
...
Total: **~$130**. Boot Raspberry Pi OS Lite (or Ubuntu Server). Follow [§2.6 Raspberry Pi](/openclaw/setup/raspberry-pi/). Connect the channels you actually use. Done.

## What we are NOT going to claim

We have not benchmarked OpenClaw on every box in the table. RAM/CPU floors are inferred from Node runtime requirements + architectural docs + community discussion. **Specific timing numbers (cold-start latency, memory-at-rest) need actual runs to confirm.** Sush will run on his M2 MacBook (Mac column promotes to `tried`) and a Pi 5 (Pi column promotes to `tried`) — those two will get real numbers first.

## What to read next

- **You picked your hardware.** → §2.2 Decision tree (planned for P0b) or jump to the right setup page below
- [§2.3 Laptop quick-start](/openclaw/setup/laptop/) — Mac / Linux desktop / Windows WSL2
- [§2.6 Raspberry Pi](/openclaw/setup/raspberry-pi/) — Pi 4 / Pi 5
- [§2.5 Azure](/openclaw/setup/azure/) — Container Apps + VM paths
- [§2.7 Docker](/openclaw/setup/docker/) — packaging method that wraps several of the above
- [§6.1 Self-hosting checklist](/openclaw/security/self-hosting-checklist/) — turn the box into a credible deployment
