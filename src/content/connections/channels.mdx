...
1. **DM policy = `pairing` by default.** Unknown senders get a one-time code, not your agent. ([§6.1 check 2](/security/self-hosting-checklist/))
...
**Discord** is the cleanest. Public Discord channels with `dmPolicy="open"` need a fully sandboxed agent (see [§6.1 check 4](/security/self-hosting-checklist/)) — *do this carefully.*

### "I want voice without a Mac"

Voice nodes are macOS / iOS / Android only. On Linux, you're text-channels-only for now. See [§1.4 drawback #6](/overview/honest-drawbacks/) for the honest take.

## What to know before pairing

- **OAuth scopes** for Slack/Discord/Teams/Google Chat are broader than they need to be by default. Read what you're granting before clicking through. Scope minimisation is [§6.2 signal #4](/security/plugin-trust-signals/).
...
- [§3.1 Index](/connections/) — patterns vs packages framing
- [§3.3 Tools](/connections/tools/) — what the agent does (built-in tools)
- [§3.4 Models](/connections/models/) — the brain choice
- [§4 Plugins](/plugins/) — specific channel connector field notes (Slack et al)
- [§6.1 Self-hosting checklist](/security/self-hosting-checklist/) — security posture for channels
