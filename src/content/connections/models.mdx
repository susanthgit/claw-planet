...
The boring answer first: **start on Sonnet, set a cheap fallback, and only reach for Opus / GPT-5.5 / Gemini Pro when your actual workload proves it needs them.** Most of the picks below are corner cases on top of that.
...
- **`openai/gpt-5.5`** — OpenAI's heavy-reasoning tier per GitHub Copilot's model list — multi-step problem solving and architecture-level analysis.
- **`google/gemini-3.1-pro-preview`** — Google's current Pro preview (the docs default for new Gemini 3 code). For a stable fallback, use `google/gemini-2.5-pro` — a previous generation (1M context), supported until 16 October 2026 shutdown.
...
- **`google/gemini-3.1-flash-lite`** — Google's lightest Gemini 3 tier, stable since 7 May 2026. A low-cost option for high-volume chat and simple classification; verify current pricing before relying on it as the cheap default. Stable Gemini 2.5 alternative: `google/gemini-2.5-flash`.
