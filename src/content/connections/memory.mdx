...
verificationNote: "Sourced from docs.openclaw.ai/concepts/memory and the three engine sub-pages. Specific examples of memory shaping are inferred from typical agent runtime patterns. Per-vendor context-window numbers (line 47) verified against Anthropic + Microsoft Foundry public docs."
...
- Workspace bootstrap files (SOUL.md, AGENTS.md, USER.md, etc. — see [§1.2 Concepts](/openclaw/overview/concepts/))
- Memory snippets injected by the [active memory system](https://docs.openclaw.ai/concepts/active-memory)

Active memory is **constrained by the model's context window** — and that varies by model. Claude Sonnet 4.6 is 1M tokens, Haiku 4.5 is 200K. GPT-5 has a 400K-token context window (with up to 272K usable for input). Big numbers but they fill faster than you'd think — a long conversation plus heavy workspace files plus injected memory plus a long tool result can hit the limit.
...
The simplest engine. Memory lives in the workspace, mainly inside **AGENTS.md** (which the runtime treats as "operating instructions + memory" — see [§1.2 Concepts](/openclaw/overview/concepts/)).
...
- [§1.2 Concepts](/openclaw/overview/concepts/) — workspace files (where built-in memory lives) and compaction
- [§3.3 Tools](/openclaw/connections/tools/) — `read` and `write` are how memory gets touched
- [§6.3 Practical patterns](/openclaw/security/practical-patterns/) — memory poisoning (pattern #4)
- [§1.4 Honest drawbacks](/openclaw/overview/honest-drawbacks/) — drawback #2 (cold start) is partially a memory issue
