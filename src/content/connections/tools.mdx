...
> *Security note:* `exec` is the most-restricted built-in tool in default sandbox setups. Non-`main` sessions running in Docker can't escape the container. Read [§6.3 pattern #5](/openclaw/security/practical-patterns/) for the injection failure mode.
...
The MCP layer is how OpenClaw connects to the broader MCP plugin world. See [§4 Plugins](/openclaw/plugins/) for specific MCP field notes.
...
The sane default: **sandbox non-main + deny dangerous tools in non-main**. See [§6.1 check 4 + check 5](/openclaw/security/self-hosting-checklist/).
...
- [§3.4 Models](/openclaw/connections/models/) — what the model brings to tool selection
- [§4 Plugins](/openclaw/plugins/) — specific tool plugins (skills + MCP) reviewed
- [§6.1 Self-hosting checklist](/openclaw/security/self-hosting-checklist/) — tool exec security posture
- [§5 Use cases](/openclaw/use-cases/) — recipes that show tools used in anger
