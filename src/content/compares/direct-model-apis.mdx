...
section: "Compare"
sectionNumber: "7.4"
description: "Three direct model API surfaces a production builder picks between. Claude API · Gemini API · Microsoft Foundry. OpenAI's direct API is out of scope here — Foundry covers the Azure-governed path to GPT-5 and the broader Foundry catalogue."
status: "published"
verificationState: "sourced"
verificationNote: "Vendor pages were fetched 15 May 2026. Pricing pages render dollar values as JavaScript on Foundry's side — exact MTok values for Azure-direct GPT models couldn't be pinned from static HTML. All other facts sourced from canonical docs."
...
- **Microsoft Foundry** — Azure platform-as-a-service that hosts 1,900+ models including Microsoft's GPT-5 series, Claude, Mistral, Llama, and more. Foundry supersedes the older "Azure AI Studio" / "Azure AI Foundry" brand and absorbs Azure OpenAI — the GPT-5 series runs as Azure Direct Models within Foundry, and existing Azure OpenAI resources can be upgraded to Foundry resources preserving endpoints, API keys, and state.

> ⚠️ **OpenAI's direct API is intentionally excluded from this comparison.** Claw has no `/openai/api/` page — Sush prioritised the five OpenAI surfaces he actually uses (Agents SDK · Codex CLI · Apps SDK · Atlas · Custom GPTs). Builders wanting raw OpenAI models in production today typically go through Foundry (covered here, since Foundry hosts the GPT-5 / GPT-5.4 / GPT-5.5 families) or call `api.openai.com` directly with the OpenAI Python SDK. We'll add an OpenAI direct-API page if that becomes a primary surface Sush works in. Until then, this is honest about scope.
...
- **Google Gemini API** — broadly the same shape, with a recent SDK changeover most builders haven't migrated yet (the `google-generativeai` Python SDK is no longer actively maintained and **stops working on 30 November 2025**; the current SDK is `google-genai`). Streaming is raw JSON chunks (no named event types). Pricing has more dimensions (priority/standard/flex/batch inference modes, separate caching serve + storage fees).
...
- **OpenAI's direct API is comparable.** It's deliberately excluded; readers wanting OpenAI in production should go to [Foundry](/microsoft/foundry/) (for the Azure-governed path) or `api.openai.com` directly (which we don't cover on Claw yet).
