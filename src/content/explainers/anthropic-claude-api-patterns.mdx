...
The model is constrained to output valid JSON. No prose wrapping, no code fences.

### Even better: JSON schema mode
...
The output is guaranteed to match the schema. If Claude can't produce a valid output, it errors rather than returning garbage.
...
Cache writes are slightly more expensive than fresh input. Cache reads are ~10% of fresh input. Break-even is typically at **2 calls hitting the same cache.** Anything beyond is pure savings.

## Pattern 4 — Message batches (async bulk work)

For when you have 1,000 prompts to run and they're not time-sensitive.
...
batch = client.messages.batches.create(
    requests=[
        {"custom_id": "ticket_1", "params": {"model": "claude-sonnet-4-5", "max_tokens": 200, "messages": [{"role": "user", "content": "Classify: ..."}]}},
        {"custom_id": "ticket_2", "params": {"model": "claude-sonnet-4-5", "max_tokens": 200, "messages": [{"role": "user", "content": "Classify: ..."}]}},
