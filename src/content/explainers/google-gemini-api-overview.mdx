...
3. **There's a new Interactions API coming.** The quickstart says: *"new models beyond the core mainline family, along with new agentic capabilities and tools, will launch exclusively on the Interactions API."* The classic `generateContent` shape stays supported, but if you're building new agent tools today, look at Interactions API patterns first.
...
- **Search grounding has explicit pricing.** Unlike most LLM APIs where "search" is opaque, Gemini's Google Search grounding bills at **$14 per 1,000 search queries** (after **5,000 free prompts per month**, shared across Gemini 3 models). Every grounded answer can return `groundingMetadata` showing which queries were issued and which sources were cited. That's unusual transparency in this space.

## How it compares (quick)

| | **Gemini API** | **OpenAI API** | **Anthropic API** |
|---|---|---|---|
| Default modern model | `gemini-2.5-flash` (stable) / `gemini-3-flash-preview` (current docs) | GPT-5 family | Claude Sonnet 4.5 |
| Context window (top tier) | 1M tokens | ~400K | 200K |
| Free tier | Yes — generous, multiple models | $5 one-off API credit | None |
