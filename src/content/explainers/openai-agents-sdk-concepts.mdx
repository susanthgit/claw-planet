...
## The nine primitives at a glance
...
The rest of this page walks through each with the minimum code to feel it.
...
- **OpenAI-hosted tools** — web search, file search, code interpreter, computer use. These run on OpenAI's side and don't need your infrastructure.
...
The SDK ships built-in support for pausing a run, asking a human, and resuming. The full pattern is in the [docs](https://openai.github.io/openai-agents-python/human_in_the_loop/); the key shape is: a tool raises a pause, your app surfaces the question, you call `Runner.resume` with the human's answer.
