...
We'll catalogue specific skills in [§4 Plugins](/openclaw/plugins/) with field notes that declare what we checked. But the *category-level* drawback is real: the marketplace's safety surface is mostly your own diligence.
...
> *Why this matters:* if you connect a public Discord channel or open DM to the agent, **change the sandbox default first** ([§6.1 Self-hosting checklist](/openclaw/security/self-hosting-checklist/)). Don't run with the default if untrusted input can reach the agent.
...
- [§6.1 Self-hosting checklist](/openclaw/security/self-hosting-checklist/) — turn drawback #4 into an action plan
- [§7.1 OpenClaw vs MCP-based stacks](/compare/openclaw-vs-mcp-stacks/) — drawback #1 (single-user shape) compared head-to-head
- [§2 Setup](/openclaw/setup/) — drawback #5 (operational floor) is where the setup pages spend most of their pages
