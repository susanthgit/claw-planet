...
On first launch, it opens a browser to `claude.ai`. You sign in (or pick the account you're already in), confirm the device, and the CLI gets a token. Token lives at `~/.claude/credentials.json` (Linux/macOS) or `%USERPROFILE%\.claude\credentials.json` (Windows). It's encrypted at rest with the OS keychain on macOS; on Windows + Linux it's a JSON file with a refresh token.
