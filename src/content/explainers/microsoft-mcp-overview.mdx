...
- **Approval model:** agents support `require_approval: "always"` — pauses and requests human approval before invoking a tool call. Recommended for write operations.
...
| M365 Copilot (via DA + ATK) | depends on plugin | depends | ✅ typical |
| Windows ODR | ✅ | — | — |
...
Microsoft's MCP coverage is **broad** — it ships across more first-party surfaces than any other vendor's stack today. But the per-surface differences are real. Don't assume a server that works in VS Code will work in Copilot Studio (transport mismatch — Stdio vs Streamable-HTTP-only). Don't assume the same auth model works everywhere (the GitHub Copilot cloud agent's lack of OAuth catches teams every month). And don't assume "M365 Copilot supports MCP" — it doesn't, directly; you need a declarative agent wrapper.
...
- **[Azure AI Foundry](/microsoft/foundry/)** — for Agent Service's MCP tool type.
