...
| `Dataverse` | Dataverse table search. Requires `host_name`, `skill` (exported from Copilot Studio), and `tables[]`. |
...
| `EmbeddedKnowledge` | Local files packaged with the agent (added v1.6). **Status: not yet available** — the schema accepts it; the runtime hasn't shipped. |
