...
| `atk preview` | Open the agent inside M365 Copilot / Teams / Outlook for live testing. Must come after provision + deploy. |
...
| `EmbeddedKnowledge` | Files packaged with the agent (added v1.6; docs flag "not yet available" — schema present, runtime not yet) |
...
The licensing slice is where readers most often get burnt. The short version: **WebSearch, CodeInterpreter, and GraphicArt are the only capabilities available without a Copilot license or pay-as-you-go billing.** Everything else needs either a per-tenant pay-as-you-go (Copilot Credits) policy or a full Microsoft 365 Copilot user license.
