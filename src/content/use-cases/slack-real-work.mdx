...
A Slack bot that **reads your team's standup channel, posts a daily summary every morning at 9am, and can answer ad-hoc questions about what was discussed**. About 60 minutes once you've got hello-world ([§5.1](/openclaw/use-cases/hello-world-laptop/)) working.
...
- OAuth scopes (the onboarder asks for the *minimum* — see [§6.2 signal #4](/openclaw/security/plugin-trust-signals/))
...
`allowChannels` is your way of scoping the agent: only `#standup` will reach it (other channels' messages don't even get processed). This is the [§6.1](/openclaw/security/self-hosting-checklist/) hygienic shape.
...
For this to work well, the agent needs **memory**. The default builtin engine works for short windows (a few days); for longer recall, set up [§3.5 QMD memory](/openclaw/connections/memory/) so the agent has semantic search over historical messages.
...
- [§5.3 WhatsApp FAQ answerer](/openclaw/use-cases/whatsapp-faq/) (planned) — similar shape for customer-facing
- [§5.4 Pi-hosted always-on home agent](/openclaw/use-cases/pi-home-agent/) — host this on a Pi instead of your laptop
- [§3.5 Memory](/openclaw/connections/memory/) — for the recall capability
- [§6.1 Self-hosting checklist](/openclaw/security/self-hosting-checklist/) — Slack-specific hardening
