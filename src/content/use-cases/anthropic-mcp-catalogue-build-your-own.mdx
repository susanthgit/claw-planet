...
An MCP server you wrote, exposing one tool (`get_weather` — the classic example), wired into Claude Code, returning real values when Claude asks. About 30 minutes for someone who's written a CLI before. The same shape extends to any tool / resource / prompt you want to expose.
...
Same as Python — use the `mcp` CLI inspector (works against any language), then configure Claude Code:
...
Just add more `@mcp.tool()` decorated functions (Python) or `server.tool()` calls (TS). The host shows all of them; the model picks which to call.
