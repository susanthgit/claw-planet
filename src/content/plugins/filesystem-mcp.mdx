...
verificationNote: "Field note assembled from public Filesystem MCP source + community deployment patterns. Sush has not yet wired Filesystem MCP into his OpenClaw install. Promotes to tested-by-sush when he runs it through his Pi or Mac with a real file tree."
...
These layer alongside OpenClaw's built-in `read`/`write`/`edit`/`exec` ([§3.3 Tools](/connections/tools/)). The difference: built-ins default to the workspace; Filesystem MCP can be scoped to **any directory you allow it**.
...
1. **Adversarial input via channels.** If your agent is reachable from any channel ([§3.2](/connections/channels/)) and Filesystem MCP is scoped to your home directory, prompt injection could exfiltrate `~/.ssh/id_rsa` or similar. **Mitigation:** scope tightly. `~/projects/` not `~/`.
...
|---|---|
| **Category** | MCP server / tool extension |
| **Layer** | Tools ([§3.3](/connections/tools/)) |
